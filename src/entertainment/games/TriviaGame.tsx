...
import { ChevronRight, Check, X as XIcon, Zap, Coins, RotateCcw, Home, BookOpen } from 'lucide-react';
import { BASE_TRIVIA_QUESTIONS, TriviaQuestion } from '../data/triviaData';
...
// Reward tuning: a correct answer nets 10 XP and 3 game coins; a wrong
// one nets 2 XP as a consolation for playing (no coins). Game coins are
// a separate currency from booking loyalty points.
...
export default function TriviaGame({ currentUser, onBack, onUserUpdated }: TriviaGameProps) {
  // Fresh 5-question sample per mount so re-playing gives different questions.
  const questions = useMemo<TriviaQuestion[]>(
    () => shuffle(BASE_TRIVIA_QUESTIONS).slice(0, QUESTIONS_PER_ROUND),
  ...
  const handleNext = async () => {
    ...
    // Round complete — apply the reward once
    if (!rewardApplied) {
      setAwarding(true);
      const wrong = questions.length - score;
      const totalXp = score * XP_PER_CORRECT + wrong * XP_PER_WRONG;
      const totalCoins = score * COINS_PER_CORRECT;
      const result = await awardGameReward(
        totalXp,
        totalCoins,
        `أسئلة كتابية — ${score}/${questions.length} إجابات صحيحة`,
      );
      if (result) {
  ...
  const restart = () => {
    // Simplest reset: bounce back to the hub so a new game mounts fresh
    onBack();
  };
      ...
      <div className="max-w-lg mx-auto px-4 sm:px-6 py-5 space-y-4" dir="rtl">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-black text-slate-200">أسئلة كتابية</span>
          </div>
