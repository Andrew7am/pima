...
const QR_CSS = `*{font-family:'Segoe UI',Tahoma,sans-serif;box-sizing:border-box}body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#F8F9FB}
...
.n{font-size:11px;color:#5B6B8C;margin-top:16px}`;
...
export async function openBookingQrWindow(bookingId: string, guest: string, houseName: string, ref: string) {
  ...
  const body = `<div class="c"><div class="h">${escapeHtml(houseName)}</div><div class="s">رمز الوصول — بيما</div>
<img src="${escapeHtml(dataUrl)}" alt="QR"/><div class="g">${escapeHtml(guest)}</div><div class="r">#${escapeHtml(ref)}</div>
<div class="n">اعرض هذا الرمز عند الوصول لتسجيل دخولك فورًا.</div></div>`;

  openPrintWindow({
    title: `رمز الوصول ${ref}`,
    css: QR_CSS,
    body,
    autoPrint: false,
    blockedMessage: 'من فضلك اسمح بالنوافذ المنبثقة لعرض الرمز.',
  });
}
